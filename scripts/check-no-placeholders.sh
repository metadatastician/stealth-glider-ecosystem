...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "stealth-glider-ecosystem"`
...
# an instantiation. The template submitted the literal `stealth-glider-ecosystem` as its own
...
# is set to `owner/stealth-glider-ecosystem` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `stealth-glider-ecosystem-2`,
